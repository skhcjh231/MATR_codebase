...
def build_transformer(args):    
    encoder = TransformerEncoder(
        TransformerEncoderLayer(d_model=args.hidden_dim,
                                nhead=args.e_nheads,
    ...
    decoder = TransformerDecoder(
        TransformerDecoderLayer(d_model=args.hidden_dim,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, v_dim, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, batch_first=False):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(d_model, nhead, vdim=v_dim, dropout=dropout, batch_first=batch_first)
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, vdim=v_dim, dropout=dropout, batch_first=batch_first)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
